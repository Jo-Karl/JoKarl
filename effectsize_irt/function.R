invariance_eff <-  function(df, items, group, nodewidth = .01, intercept_fix = 1, lowerLV = -5, upperLV = 5,...) {
  ...
  outNoninvariance_f <- lavaan::cfa(cfa_model_f, data = df, group = group, std.lv = T,
                       meanstructure = T, ...)
...
multi_group_eff <- function(df, group, items, eff_sizes = c("SDI2", "UDI2", "WSDI", "WUDI", "dmacs")){
